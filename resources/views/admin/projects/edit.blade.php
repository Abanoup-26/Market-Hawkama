...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.projects.update", [$project->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="date">{{ trans('cruds.project.fields.date') }}</label>
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $project->date) }}">
                @if($errors->has('date'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('collected') ? 'is-invalid' : '' }}" type="number" name="collected" id="collected" value="{{ old('collected', $project->collected) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('goal') ? 'is-invalid' : '' }}" type="number" name="goal" id="goal" value="{{ old('goal', $project->goal) }}" step="0.01" required>
                @if($errors->has('goal'))
                    <div class="invalid-feedback">
                        {{ $errors->first('goal') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.project.fields.goal_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label class="required" for="file">{{ trans('cruds.project.fields.file') }}</label>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('short_description') ? 'is-invalid' : '' }}" name="short_description" id="short_description" required>{{ old('short_description', $project->short_description) }}</textarea>
...
@if(isset($project) && $project->image)
      var files = {!! json_encode($project->image) !!}
          for (var i in files) {
          var file = files[i]
...
@if(isset($project) && $project->file)
      var file = {!! json_encode($project->file) !!}
          this.options.addedfile.call(this, file)
      file.previewElement.classList.add('dz-complete')
